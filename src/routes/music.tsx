...
function Music() {
  ...
  const counts = useMemo(() => dailySongs.reduce<Record<string, number>>((a, s) => ({ ...a, [s.id]: a[s.id] ?? 0 }), {}), []);
  const recommend = () => {
  ...
  return <AppShell>
    <PageTitle kicker="CHAON MUSIC" title="오늘 뭐 들을까?" sub="매일 한 곡을 골라요. 듣고 싶은 노래도 직접 추천할 수 있어요." />

    <section className="mt-5 rounded-[30px] bg-primary p-5 text-primary-foreground shadow-card">
      <div className="flex items-center gap-2"><Music2 size={20} /><p className="text-[10px] font-bold tracking-[0.18em] text-primary-foreground/65">TODAY'S VOTE</p></div>
      <h2 className="mt-2 font-display text-2xl">오늘의 1등곡을 정해보자</h2>
      <p className="mt-1 text-xs text-primary-foreground/70">하루에 한 번 투표하고 +1P</p>
      <div className="mt-4 space-y-2">{dailySongs.map((song) => {
        ...
        return <button key={song.id} type="button" disabled={!!voted} onClick={() => { if (voteMusic(date, song.id)) toast.success("투표 완료! +1P"); }} className={`tap w-full rounded-2xl p-3 text-left ${selected ? "bg-white text-primary" : "bg-white/12 text-primary-foreground"}`}>
          <div className="flex items-center gap-3"><span className={`grid size-10 place-items-center rounded-xl ${selected ? "bg-primary/10" : "bg-white/10"}`}><Music2 size={17} /></span><span className="min-w-0 flex-1"><span className="block font-display text-base">{song.title}</span><span className={`block text-[11px] ${selected ? "text-primary/60" : "text-primary-foreground/60"}`}>{song.artist} · {song.mood}</span></span>{selected ? <Check size={19} /> : <span className="text-xs font-bold">투표</span>}</div>
          {voted ? <div className={`mt-2 h-1.5 overflow-hidden rounded-full ${selected ? "bg-primary/10" : "bg-white/10"}`}><div className="h-full rounded-full bg-current" style={{ width: `${total ? (counts[song.id] / total) * 100 : 0}%` }} /></div> : null}
        </button>;
      })}</div>
    </section>

    <section className="mt-7 rounded-[28px] bg-card p-5 shadow-card">
      <div><p className="text-[10px] font-bold tracking-[0.18em] text-primary">REQUEST A SONG</p><h2 className="mt-1 font-display text-xl">듣고 싶은 노래 추천하기</h2><p className="mt-1 text-xs text-muted-foreground">한 화면에서 바로 추천하고, 다음 투표 후보로 모아요.</p></div>
