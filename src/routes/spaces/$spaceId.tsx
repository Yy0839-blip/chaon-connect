...
import { ArrowLeft, CalendarPlus, Check, MapPin, Sparkles } from "lucide-react";
...
function SpaceDetail() {
    ...
    return (
      <AppShell>
        <div className="py-16 text-center">
          ...
          <Link to="/spaces" className="mt-4 inline-flex rounded-2xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground">
            놀거리로 돌아가기
          </Link>
        </div>
      </AppShell>
  ...
  return (
    <AppShell>
      <Link to="/spaces" className="tap mb-4 inline-flex items-center gap-1.5 text-sm font-bold text-muted-foreground">
        ...
        <div className="p-5">
          ...
          <div className="mt-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-display text-xl">이렇게 놀아봐</h2>
            </div>
            <div className="mt-3 space-y-2.5">
              {space.highlights.map((item) => (
                <div key={item} className="flex items-start gap-2.5 rounded-2xl bg-muted/70 p-3.5 text-sm">
          ...
          <div className="mt-6 rounded-2xl bg-secondary p-4">
            ...
            <ul className="mt-2 space-y-1.5 text-xs leading-relaxed text-muted-foreground">
              {space.tips.map((tip) => <li key={tip}>· {tip}</li>)}
            </ul>
          </div>

          <Link
            to="/community"
            className="tap mt-5 flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-primary py-3.5 font-display text-base text-primary-foreground shadow-pop"
          >
            <CalendarPlus size={18} /> 이걸로 약속 만들기
          </Link>

          <p className="mt-3 text-center text-[10px] text-muted-foreground">
            사진: {space.sourceLabel} · 외부 참고 이미지
          </p>
        </div>
      </section>
    </AppShell>
