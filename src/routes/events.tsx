  ...
  head: () => ({
    meta: [
      { title: "2026 연말 이벤트 · 공모전·플리마켓·파티 | 차온 CHAON" },
      {
        name: "description",
        content:
          "차오름 연말 이벤트 안내. 공모전, 플리마켓, 연말파티까지 우리들의 연말을 직접 만들어요.",
      },
      { property: "og:title", content: "2026 CHAON YEAR-END" },
      {
        property: "og:description",
        content: "공모전·플리마켓·연말파티, 우리들의 연말을 만들어보자.",
      },
    ],
  }),
  component: Events,
  ...
  return (
    <AppShell>
      ...
      <section id="contest" className="mt-7 scroll-mt-24 rounded-3xl bg-limeGrad p-5 shadow-card">
        ...
        <h2 className="mt-1 font-display text-2xl text-lime-foreground">
          🏆 우리 동네를 우리가 만든다
        </h2>
        <p className="mt-1.5 text-sm text-lime-foreground/80">
          영상, 사진, 그림, 디자인, 댄스, 아이디어 중 하나만 골라 제출하면 끝. 수상작은 연말파티에서
          공개돼.
        </p>
      ...
      <section
        id="market"
        className="mt-4 scroll-mt-24 rounded-3xl bg-sky p-5 text-sky-foreground shadow-card"
      >
      ...
      <section
        id="party"
        className="mt-4 scroll-mt-24 rounded-3xl bg-night p-5 text-navy-foreground shadow-card"
      >
      ...
      <div className="mt-6">
        <ShareCTA
          label="친구랑 같이 신청하기"
          message="차오름 연말 이벤트 같이 나갈래? 공모전에 플리마켓까지 있어!"
        />
      </div>
    </AppShell>
  );
