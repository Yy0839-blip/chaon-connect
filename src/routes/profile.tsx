...
export const Route = createFileRoute("/profile")({ head: () => ({ meta: [{ title: "내 프로필 · 차온 CHAON" }, { name: "description", content: "내 포인트, 배지, 참여 기록과 내가 만든 약속을 확인하세요." }] }), component: Profile });
...
function Profile() {
  ...
  return <AppShell>
    ...
    <section className="rise mt-5 rounded-3xl bg-night p-5 text-navy-foreground shadow-card"><div className="flex items-center gap-3"><label className="tap relative grid size-20 shrink-0 cursor-pointer place-items-center overflow-hidden rounded-[24px] bg-white/15 text-3xl ring-2 ring-white/15" title="갤러리에서 사진 선택">{avatar.startsWith("data:image/") ? <img src={avatar} alt="내 프로필 사진" className="size-full object-cover" /> : avatar}<span className="absolute bottom-1 right-1 grid size-7 place-items-center rounded-full bg-primary text-primary-foreground shadow-pop"><ImagePlus size={14} /></span><input type="file" accept="image/*" className="sr-only" onChange={(e) => void handlePhoto(e.target.files?.[0])} /></label><span><span className="block font-display text-2xl">{nickname || "익명의 차오름러"}</span><span className="block text-xs text-navy-foreground/60">차오름 방문 {visits}회</span><span className="mt-1 block text-[10px] font-bold text-lime">사진을 눌러 갤러리에서 변경</span></span></div><div className="mt-4 grid grid-cols-3 gap-2 text-center">{[{ l: "포인트", v: points }, { l: "배지", v: badges.length }, { l: "미션", v: `${doneMissions.length}/${missions.length}` }].map((i) => <div key={i.l} className="rounded-2xl bg-white/10 py-3"><p className="font-display text-xl text-lime">{i.v}</p><p className="text-[11px] text-navy-foreground/60">{i.l}</p></div>)}</div><button type="button" onClick={() => setProfile("", avatar)} className="mt-4 w-full py-2 text-xs font-bold text-navy-foreground/60">닉네임 다시 정하기</button></section>
    ...
    <div className="mb-3 mt-7 flex items-end justify-between"><h2 className="font-display text-xl">내가 만든 약속</h2><Link to="/community" className="text-xs font-bold text-primary">커뮤니티 →</Link></div>
