...
type State = { nickname:string; avatar:string; profileTags:string[]; points:number; badges:string[]; doneMissions:string[]; joinedPrograms:string[]; joinedEvents:string[]; visits:number; posts:Post[]; meetups:Meetup[]; musicVotes:Record<string,string>; musicVoteCounts:Record<string,number>; musicRecommendations:MusicRecommendation[] };
const KEY="chaon.state.v5";
const initial:State={nickname:"",avatar:"🐤",profileTags:["친구랑 노는 걸 좋아해"],points:240,badges:["첫 방문","게임왕","친구왕"],doneMissions:[],joinedPrograms:[],joinedEvents:[],visits:4,posts:seedPosts,meetups:[],musicVotes:{},musicVoteCounts:{},musicRecommendations:[]};
...
export function ChaonProvider({children}:{children:ReactNode}){
 ...
 const reloadShared=useCallback(async(uid:string)=>{if(!supabase)return;const [postsR,meetupsR,recsR,votesMineR,votesAllR,likesR]=await Promise.all([
   ...
   supabase.from("music_votes").select("vote_date,song_id"),
   supabase.from("post_likes").select("post_id").eq("user_id",uid)
 ]);
 const liked=new Set((likesR.data||[]).map((x:any)=>x.post_id));
 const counts:Record<string,number>={};(votesAllR.data||[]).forEach((v:any)=>{counts[`${v.vote_date}:${v.song_id}`]=(counts[`${v.vote_date}:${v.song_id}`]||0)+1});
 ...
 if(votesMineR.data) setState(s=>({...s,musicVotes:Object.fromEntries(votesMineR.data.map((v:any)=>[v.vote_date,v.song_id]))}));
 setState(s=>({...s,musicVoteCounts:counts}));
 },[]);
 ...
 useEffect(()=>{if(!supabase)return;let mounted=true;let channel:any;const start=async()=>{
   ...
   channel=supabase.channel(`chaon-live-${uid}`).on("postgres_changes",{event:"*",schema:"public",table:"posts"},()=>void reloadShared(uid)).on("postgres_changes",{event:"*",schema:"public",table:"post_likes"},()=>void reloadShared(uid)).on("postgres_changes",{event:"*",schema:"public",table:"meetups"},()=>void reloadShared(uid)).on("postgres_changes",{event:"*",schema:"public",table:"meetup_members"},()=>void reloadShared(uid)).on("postgres_changes",{event:"*",schema:"public",table:"music_votes"},()=>void reloadShared(uid)).on("postgres_changes",{event:"*",schema:"public",table:"music_recommendations"},()=>void reloadShared(uid)).subscribe();
 };void start();return()=>{mounted=false;if(channel)void supabase.removeChannel(channel)},[reloadShared]);
 const setProfile=useCallback((nickname:string,avatar:string)=>{const clean=nickname.trim().slice(0,20);setState(s=>{const next={...s,nickname:clean,avatar};if(userId)persistProfile(userId,next);return next})},[userId,persistProfile]);
 ...
 const toggleMeetup=useCallback((id:string)=>{if(!supabase||!userId||id.startsWith("local-"))return false;const meetup=state.meetups.find(m=>m.id===id);if(!meetup||meetup.joinedPeople>=meetup.maxPeople)return false;setState(s=>({...s,meetups:s.meetups.map(m=>m.id===id?{...m,joinedPeople:m.joinedPeople+1}:m)}));void supabase.rpc("join_meetup",{p_meetup_id:id}).then(({data,error})=>{if(error||!data)void reloadShared(userId);else void reloadShared(userId)});return true},[userId,state.meetups,reloadShared]);
 const voteMusic=useCallback((date:string,songId:string)=>{if(state.musicVotes[date])return false;setState(s=>({...s,musicVotes:{...s.musicVotes,[date]:songId},musicVoteCounts:{...s.musicVoteCounts,[`${date}:${songId}`]:(s.musicVoteCounts[`${date}:${songId}`]||0)+1},points:s.points+1}));if(supabase&&userId)void supabase.from("music_votes").insert({user_id:userId,vote_date:date,song_id:songId}).then(({error})=>{if(error)void reloadShared(userId)});return true},[state.musicVotes,userId,reloadShared]);
 const addMusicRecommendation=useCallback((title:string,artist:string,date:string)=>{const t=title.trim().slice(0,60),a=artist.trim().slice(0,60);if(!t||!a)return;const recommender=state.nickname||"익명의 차오름러";if(supabase&&userId)void supabase.from("music_recommendations").insert({user_id:userId,title:t,artist:a,vote_date:date,recommender}).then(({error})=>{if(!error)void reloadShared(userId)});else setState(s=>({...s,musicRecommendations:[{id:`song-${Date.now()}`,title:t,artist:a,date,recommender},...s.musicRecommendations].slice(0,30)}))},[userId,state.nickname,reloadShared]);
